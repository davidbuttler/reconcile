...
public class PairedTTester implements OptionHandler {
  ...
  protected class Resultset {
	...
	// compact the string
    tempResult = Utils.removeSubstring(tempResult, "reconcile.weka.classifiers.");
    tempResult = Utils.removeSubstring(tempResult, "reconcile.weka.filters.");
    tempResult = Utils.removeSubstring(tempResult, "reconcile.weka.attributeSelection.");
	result += tempResult;
      }
      return result.trim();
    }
  ...
  if (result.startsWith("reconcile.weka.classifiers.")) {
    result = result.substring("reconcile.weka.classifiers.".length());
    }
    return result.trim();
  }
  ...
  public String multiResultsetSummary(int comparisonColumn)
	...
	result += "c";
      ...
      titles += ' ' + Utils.padLeft("" + (char)('a' + i % 26),
				    resultsetLength * 2 + 3);
    }
    if (m_latexOutput) {
      result += "}\\\\\n\\hline\n";
