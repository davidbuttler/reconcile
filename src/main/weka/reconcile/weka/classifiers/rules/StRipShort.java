 ...
 * Preferences - Java - Code Style - Code Templates
 */
public class StRipShort implements OptionHandler, AdditionalMeasureProducer,
    WeightedInstancesHandler{

  /** The limit of description length surplus in ruleset generation */
  private static double MAX_DL_SURPLUS = 64.0;

  /** A constant that controls how the algorithm works. In several places, the JRep
   *  implementation differs from ripper, such as the loss ratio, and prunning. When
   *  this variable is set to true, this places are handled like RIPPER.
   */
  private boolean m_RipperStyle = false;
  
  /** Whether to use positive correlation (accuracy) instead information gain when growing rules*/
  ...
  public Enumeration listOptions() {
    ...
    newVector.addElement(new Option("\tSet the maximum number of values for numeric\n"
        + "\tattributes. (Default: 100)", "V", 1, "-V <num_values>"));
    newVector.addElement(new Option("\tSet whether turn on the\n"
        + "\tdebug mode (Default: false)", "D", 0, "-D"));
    newVector.addElement(new Option("\tUnset default way that rules are grown\n"
        + "\tduring optimization (Default: true)", "R", 0, "-R"));
    ...
    newVector.addElement(new Option("Whether NOT prune Ripper style\n"
        + "\t(default: not)", "Q", 0, "-Q"));
    newVector.addElement(new Option("Whether NOT use positive correlation instead of info gain\n"
        + "\t(default: use info gain)", "C", 0, "-C"));
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    String fnString = Utils.getOption('M', options);
    if(fnString.length() != 0)
      m_saveFilename = fnString;
    else
      m_saveFilename = null;
    ...
    m_UsePosCor = Utils.getFlag('C', options);
    m_RipperStyle = Utils.getFlag('Q', options);
    String ratioString = Utils.getOption('L', options);
    if(ratioString.length() != 0){
      float loss_ratio = Float.parseFloat(ratioString);
      if(m_RipperStyle){
        m_FP_cost = 2/(loss_ratio+1);
        m_FN_cost = (2*loss_ratio)/(loss_ratio+1);
        m_Ratio = 1;
        m_RatioOrig = loss_ratio;
      }else{
        m_FP_cost =1;
        m_FN_cost =1;
        m_RatioOrig = m_Ratio = loss_ratio;
      }
    }else
      m_Ratio = 1;
    
    String valuesString = Utils.getOption('V', options);
    if(valuesString.length() != 0)
      m_MaxNumValues = Integer.parseInt(valuesString);
    else
      m_MaxNumValues = 100;
    ...
    m_DefaultOpt = !Utils.getFlag('R', options);
  }
   ...
   */
  public StRuleStatsShort getRuleStats(int pos) {
    return (StRuleStatsShort)m_RulesetStats.elementAt(pos);
  }
  ...
  private abstract class Antd implements WeightedInstancesHandler, Copyable,
    ...
    public abstract InstancesShort[] splitData(InstancesShort data,
        double defAcRt, double cla);

    public abstract boolean covers(InstanceShort inst);
    
    public abstract boolean match(InstanceShort inst);
    ...
    public void setAttrValue(short v) {
      value = v;
    }
    ...
    public void setBestAttrValue(short currentValue) {
  ...
  private class NumericAntd extends Antd{
    ...
    /* Value for <= */
    static final short GE = 1;

    /* Constructor */
    public NumericAntd(AttributeShort a) {
      super(a);
      splitPoint = Short.MIN_VALUE;
      value = LE;
    }

    /* Get split point of this numeric antecedent */
    public short getSplitPoint() {
      return splitPoint;
    }

    /* Set the split point of this numeric antecedent */
    public void setSplitPoint(short sp) {
      splitPoint = sp;
    }
     ...
     */
    public InstancesShort[] splitData(InstancesShort insts, double defAcRt,
        double cl) {
      InstancesShort data = insts;
      int total = data.numInstances();// Total number of instances without
      // missing value for att

      short split = 1; // Current split position
      short prev = 0; // Previous split position
      short finalSplit = split; // Final split position
      maxInfoGain = 0;
      value = 0;
      ...
      for(int x = 0; x < data.numInstances(); x++){
        InstanceShort inst = data.instance(x);
      ...
      for(; split <= total; split++){
        if((split == total) || (data.instance(split).value(att) > // Can't
          ...
          for(int y = prev; y < split; y++){
            InstanceShort inst = data.instance(y);
      ...
      /* Split the data */
      ModifiedInstancesShort[] splitData = new ModifiedInstancesShort[2];
      splitData[0] = new ModifiedInstancesShort(data, 0, finalSplit);
      splitData[1] = new ModifiedInstancesShort(data, finalSplit, total
          - finalSplit);
     ...
     *         this antecedent
     */
    public boolean covers(InstanceShort inst) {
      boolean isCover = true;
      if(!inst.isMissing(att)){
        if(value == LE){ // First bag
     ...
     *         this antecedent
     */
    public boolean match(InstanceShort inst) {
      boolean isCover = true;
      if(!inst.isMissing(att)){
        if(value == LE){ // First bag
          if(att.getOriginalValue(inst.value(att)) > splitValue)
            isCover = false;
        } else if(att.getOriginalValue(inst.value(att)) < splitValue) // Second bag
          isCover = false;
      } else
        isCover = false;
    ...
    public String toString() {
      String symbol = ((int)value == LE)?" <= ":" >= ";
      String attrValue = splitPoint==Short.MIN_VALUE?Utils.doubleToString(splitValue, 6):Utils.doubleToString(att.getOriginalValue(new Short(splitPoint)),6);
      
      return (att.name() + symbol + attrValue);
    }
    
    public String pPrint(){
   	 return toString()+" "+accu;
    }
  ...
  private class NominalAntd extends Antd{
    ...
    /* Constructor */
    public NominalAntd(AttributeShort a) {
      super(a);
     ...
     * @return the array of data after split
     */
    public InstancesShort[] splitData(InstancesShort data, double defAcRt,
        double cl) {
      int bag = att.numValues();
      InstancesShort[] splitData = new ModifiedInstancesShort[bag];

      for(int x = 0; x < bag; x++){
        splitData[x] = new ModifiedInstancesShort(data, data.numInstances());
        accurate[x] = 0;
        coverage[x] = 0;
      }
  ...
  protected class RipperRule implements WeightedInstancesHandler, Copyable, Serializable{
    ...
    public void addAntes(Antd a){
   	 m_Antds.addElement(a);
    }
     ...
     *         this rule
     */
    public boolean covers(InstanceShort datum) {
      boolean isCover = true;
     ...
     */
    private double computeDefAccu(InstancesShort data) {
      double defAccu = 0;
      AttributeShort covered = m_Covered;
      int positive = covered.indexOfValue("+");
      for(int i = 0; i < data.numInstances(); i++){
        InstanceShort inst = data.instance(i);
     ...
     */
    public InstancesShort apply(InstancesShort data, boolean useCovered) {
      AttributeShort predClass = m_PredictedClass;
      short positive = predClass.indexOfValue("+");
      // System.err.println("Applying to attr " + predClass);
      AttributeShort covered = m_Covered;
      int numCovered = 0, numNewCovered = 0, totalCovered = 0;
      for(int i = 0; i < data.numInstances(); i++){
        InstanceShort inst = data.instance(i);
        short cov = inst.value(covered);
        if(covers(inst)){
      ...
      if(m_Debug)
        System.err.println("After apply: " + numCovered + " (" + totalCovered
            + ") total; " + numNewCovered + " new.");
      return data;
    }
     ...
     * @return data with the pred_class attribute modified
     */
    public InstancesShort apply(InstancesShort data, int num) {
      AttributeShort predClass = m_PredictedClass;
      AttributeShort covered = m_Covered;
     ...
     * @return data with the pred_class attribute modified
     */
    private InstancesShort apply(InstancesShort data, int begin, int end) {
      if(begin <= 0)
    ...
    /**
     * Private function to apply all rules in a ruleset and compute the covered
     * attribute.
     * 
     * @param ruleset
     *          The ruleset.
     ...
     *          the data in question
     * @param endPosition
     *          The position of the first rule that is not to be applied.
     * @return data with the covered attribute modified
     */
    public InstancesShort applyWhileSkipping(FastVector ruleset,
        InstancesShort data, int index, int endPosition) {
      AttributeShort cover = m_Covered;
      int end = endPosition < ruleset.size()?endPosition:ruleset.size();
      for(int j = 0; j < data.numInstances(); j++){
        InstanceShort inst = data.instance(j);
        inst.setValue(cover, "-");
        for(int i = 0; i < end; i++){
          if(i != index){
     ...
     * @return data with the covered attribute modified
     */
    public InstancesShort apply(FastVector ruleset, InstancesShort data,
        int endPosition) {
      AttributeShort cover = m_Covered;
      int end = endPosition < ruleset.size()?endPosition:ruleset.size();
      for(int j = 0; j < data.numInstances(); j++){
     ...
     * @return data with the pred_class attribute modified
     */
    private InstancesShort apply(FastVector ruleset, InstancesShort data) {
     ...
     *          the data in question
     * @param ruleNum
     *          The index of the rule that is to be skipped.
     * @return data with the pred_class attribute modified
     */
    public InstancesShort applyWhileSkipping(FastVector ruleset,
        InstancesShort data, int ruleNum) {
      AttributeShort cover = m_Covered;
      for(int j = 0; j < data.numInstances(); j++){
        InstanceShort inst = data.instance(j);
        inst.setValue(cover, "-");
    ...
    public InstancesShort clear(InstancesShort data, boolean useCovered) {
      AttributeShort predClass = m_PredictedClass;
      AttributeShort cover = m_Covered;
      for(int i = 0; i < data.numInstances(); i++){
        InstanceShort inst = data.instance(i);
     ...
     */
    private boolean applyWithAntd(InstancesShort data, Antd ant) {
      if(m_Debug && ves_Debug)
        System.err.println("Ant " + ant);
      double covers = 0;
      AttributeShort realClass = m_RealClass;
      AttributeShort covered = m_Covered;
      short pos_index = covered.indexOfValue("+");
      for(int i = 0; i < data.numInstances(); i++){
        InstanceShort inst = data.instance(i);
        short val = inst.value(covered);
        if(val != pos_index){
          if(!ant.covers(inst))
     ...
     */
    public void grow(InstancesShort growData) throws Exception {
      //if(!(gd instanceof InstancesShort))
      //  throw new RuntimeException("Wrong Class");
      //InstancesShort growData = (InstancesShort)gd;
      if(m_Consequent == -1)
      ...
      //System.err.println("Documents: "+((ModifiedInstancesShort)growData).printDocOrder());
      if(m_Debug && ves_Debug){
        System.err.println("^^^^^^^^^^\n" + growData.toString(false));
        System.err.println("Growing rule " + this.toString(m_Class)
            + " default accuracy = " + defAccu);
        //System.err.println(growData.toString(false));
      }
      ...
      while(Utils.gr(growData.numInstances(), 0.0) && (numUnused > 0)
        ...
        if(m_Debug){
          ...
          System.err.println("\nOne condition: size = "
              + nf.format(growData.sumOfWeights()) + ". Total instances "
              + nf.format(growData.numInstances()) + ". Default accuracy "
              + nf.format(defAcRt));
        }
        /* Build one condition based on all attributes not used yet */
        while(enumAttr.hasMoreElements()){
          AttributeShort att = (AttributeShort)(enumAttr.nextElement());
            ...
            if(Utils.gr(cover, 0)){
              ...
              if(infoGain > maxInfoGain){
                if(m_Debug){
                  NumberFormat nf = NumberFormat.getInstance();
                  nf.setMaximumFractionDigits(2);
                  System.err.println("\tInfoGain = " + nf.format(infoGain)
                      + " | Accuracy = " + nf.format(antd.getAccuRate()) + "="
                      + nf.format(antd.getAccu()) + "/"
        ...
        growData = getCoveredData(growData, oneAntd);// Grow data size is
                                                     // shrinking
        ...
        //when using positive coreletion, it is possible that the algorithm gets stuck
        //adding the same antecedent over and over. This takes care of that problem.
        stop = m_UsePosCor && Utils.smOrEq(maxInfoGain,curInfoGain);
        curInfoGain = maxInfoGain;
      }
      if(m_Debug && ves_Debug)
     ...
     */
    // private InstancesShort computeInfoGain(InstancesShort instances, double
    // defAcRt,
    // Antd antd) {
    ...
    // */
    // InstancesShort[] splitData = antd.splitData(data, defAcRt, m_Consequent);
    //
     ...
     */
    private double computeInfoGainOld(InstancesShort instances, double defAcRt,
        Antd antd) {
      InstancesShort data = instances;// , result[];
      ...
      if(antd.getAttr().isNominal()){//Nominal antecedent
        ...
        double offset = 1;
        if(m_Debug)
          System.err.print("\t*Numeric antd " + antd + " with " + bag
              + " values. ");
        if(bag > m_MaxNumValues){
          //m_MaxNumAttributes = bag;
          offset = ((double)bag) / ((double)m_MaxNumValues);
          bag = m_MaxNumValues;
          if(m_Debug)
            System.err
                .println("Reducing to " + m_MaxNumValues + " values.");
        } else{
          if(m_Debug)
            System.err.println();
        }
        ...
        for(int b = 0; b < bag; b++){
          // result[v] = new ModifiedInstances(data, data.numInstances());
          // Try all possible values for the antecedent
          short value = (short)(b * offset);
          for(short le = 0; le <= 1; le++){
            NumericAntd nantd = (NumericAntd)antd;
            nantd.setAttrValue(le);
            nantd.setSplitPoint(value);
            boolean covers = applyWithAntd(data, antd);
            if(!covers){ // this antecedent doesn't cover any instances; skip
              coverage[2 * b + le] = 0;
              continue;
        ...
        for(short b = 0; b < bag; b++){
          for(short le = 0; le < 1; le++){
            ...
            if(infoGain > maxInfoGain){
              ...
              accuRate = p / t;
              val = le;
              ((NumericAntd)antd).setSplitPoint((short)(b*offset));
              ind = b;
            }
          }
        }
     ...
     * @return the info gain for the antecedent
     */
    private double computeInfoGain(InstancesShort instances, double defAcRt, Antd antd) {
    ...
    /**
     * Compute the best information gain for the specified nominal antecedent
     * 
     * @param instances
     *          the data based on which the infoGain is computed
     * @param defAcRt
     ...
     * @return the info gain for the antecedent
     */
    private double computeInfoGainNominal(InstancesShort instances,double defAcRt, Antd antd) {
      InstancesShort data = instances;// , result[];
      double maxInfoGain = 0;
      AttributeShort currentClass = m_RealClass;
      AttributeShort covered = m_Covered;
    ...
    /**
     * An encapsulating class needed for computing info gain of numeric antecedents
     * @author ves
     *
    ...
    private class SmallInstance{
      ...
      public SmallInstance(short doc, short np1, short np2, boolean pos, boolean cov){
        this.docid = doc;
        this.id1 = np1;
        this.id2 = np2;
        this.positive = pos;
     ...
     * @return the info gain for the antecedent
     */
    private double computeInfoGainNumeric(InstancesShort instances, double defAcRt, Antd antd) {
      ...
      for(int i = 0; i < data.numInstances(); i++){
        ...
        if(!isCovered && covers(current)){//if the current instance is covered by the rule so far
        ...
        positive[docid][id1][id2] = positive[docid][id2][id1] = isCovered?0:(pos?weight:(-1*weight));
      ...
      for(int i = 1; i < splits.length; i++){
        //System.err.print("|"+antd.getAttr().getOriginalValue(new Short((short)i))+"c-"+coverage[i]);
      ...
      for(short b = 0; b < bag; b++){
        ...
        //double infoGain = accurate[b] * (Utils.log2(p / t) - Utils.log2(defAcRt));
        // if (m_Debug)
        // System.err.println(antd+" => " + infoGain + "=" + accurate[x]
        // + "(*" + Utils.log2(p / t) + "-" + Utils.log2(defAcRt) + ")");
        if(infoGain > maxInfoGain){
      ...
      if(m_Debug){
        ...
        System.err.println("Computed " + accu + "/" + cover + ";True " + trueAccu + "/" + trueCover);
      }*/
      if(m_Debug)
    ...
    private InstancesShort getCoveredData(InstancesShort data, Antd antd) {
      ...
      InstancesShort result = new ModifiedInstancesShort(data, data.numInstances());
    ...
    public void prune(InstancesShort pruneData, boolean useWhole) {
      ...
      for(int x = 0; x < size; x++){
        ...
        if(m_RipperStyle){
          ...
          if(!useWhole)
            worthRt[x]= ((p+1)*m_FN_cost - (n+1)*m_FP_cost)/(p*m_FN_cost + (n+1)*m_FP_cost + 4.0);
          else{
            double fp = n;
            double fn = tn[x];
      ...
      for(int i = 0; i < worthValue.length; i++){
        if(m_Debug){
          ...
          String computation = m_RipperStyle?"(RIPPERStyle): ":"(useAccuray? " + !useWhole + "): ";
          System.err.println(i + computation + worthRt[i] + "=" + worthValue[i] + "/" + denom);
        }
  ...
  public void buildClassifier(InstancesShort instances) throws Exception {
    System.err.println("StRipShort with options "+Utils.joinOptions(getOptions())+" docs="
    ...
    oneClass: for(int y = 0; y < instances.numClasses() - 1; y++){ // For each
      ...
      if(defaultRuleStat==null)
        defaultRuleStat = new StRuleStatsShort(m_Debug&&ves_Debug, m_FP_cost, m_FN_cost);
      if(classYWeights > 0)
        defDL = defaultRuleStat.dataDL(expFPRate, 0.0, totalWeights, 0.0, classYWeights);
      else
    ...
    StRuleStatsShort defRuleStat = new StRuleStatsShort(m_Debug && ves_Debug, m_FP_cost, m_FN_cost);
    ...
    //System.out.println("Default: "+defRuleStat.toRIPPERString());
  }
  ...
  protected InstancesShort rulesetForOneClass(double expFPRate,
    ...
    if(m_UsePruning){
      for(int z = 0; z < m_Optimizations; z++){
        ...
        // newData = data;
        finalRulesetStat = new StRuleStatsShort(m_Debug && ves_Debug, m_FP_cost, m_FN_cost);
        ...
        oneRule: while(!stop && hasPositive){
          ...
          if(isResidual){
            ...
            rstats = new StRuleStatsShort(data, ruleset, m_Debug && ves_Debug, m_FP_cost, m_FN_cost);
            rstats.setNumAllConds(m_Total);
  ...
  public static void main(String[] args) {
    try{
      ...
      System.out.println("Classifier built. "+classifier.m_RulesetStats.size()+" rules.");
      if(classifier.m_saveFilename!=null){
      	PrintStream out = new PrintStream(classifier.m_saveFilename);
      	for(int i=0; i<classifier.m_RulesetStats.size()-1; i++){
      		StRuleStatsShort rulest = (StRuleStatsShort)classifier.m_RulesetStats.elementAt(i);
      		out.println(rulest.toRIPPERString());
      	}
      }
    } catch(Exception e){
      e.printStackTrace();
  ...
  /*
   * Read a classifier model from a file
   */
  public static StRipShort readClassifier(String modelFilename, InstancesShort insts) {
	  try{
	  StRipShort result = new StRipShort();
	  result.m_Ruleset = new FastVector();
     ...
     while((line = input.readLine())!=null){
   	  ...
   	  //We expect to read
   	  //the class
   	  RipperRule rule = result.new RipperRule();
   	  String ruleClass = comps[0];
   	  double clValue = ruleClass.equals("+")?1:-1;
   	  rule.setConsequent(clValue);
   	  //Correct/incorrect nums
   	  int correct = Integer.parseInt(comps[1]);
   	  int incorrect = Integer.parseInt(comps[2]);
   	  double acc = (double)correct/(double)(incorrect+correct);
   	  rule.setAccuracy(acc);
   	  //The antecedents
   	  for(int i=4; !comps[i].equals(".");i+=3){
   		  String name = comps[i];
   		  String sign = comps[i+1];
   		  String num = comps[i+2];
   		  AttributeShort at = insts.attribute(name);
   		  Antd a;
   		  if(at.isNominal()){
   			  a = result.new NominalAntd(at);
   			  a.setValue(Short.parseShort(num));
   		  }else{
   			  a = result.new NumericAntd(at);
   			  ((NumericAntd)a).setSplitValue(Double.parseDouble(num));
   			  if(sign.equals("<="))
   				  a.setValue(NumericAntd.LE);
   			  else{
   				  if(sign.equals(">=")){
   					  a.setValue(NumericAntd.GE);
   				  }else{
   	  ...
   	  System.out.println("Rule "+rule.toRIPPERString()+" "+rule.getAccuracy());
   	  result.m_Ruleset.addElement(rule);
     }
  ...
  public double classifyInstance(InstanceShort data){
	  for(int i=0; i<m_Ruleset.size();i++){
		  RipperRule r = (RipperRule)m_Ruleset.elementAt(i);
		  if(r.covers(data)){
			  double accuracy = r.getAccuracy();
			  return r.getConsequent()*accuracy;
		  }
	  }
	  throw new RuntimeException("No rule matches instance.");
  }
