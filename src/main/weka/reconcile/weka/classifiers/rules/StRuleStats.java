 ...
 * 
 */
public class StRuleStats implements Serializable {
	...
	public StRuleStats(boolean debug) {
		//m_Data = null;
		m_Ruleset = null;
		m_SimpleStats = null;
		// m_Filtered = null;
	 ...
	 *          the ruleset
	 */
	public StRuleStats(Instances data, FastVector rules) {
		this();
		//m_Data = data;
		m_Ruleset = rules;
	}
	 ...
	 *          the ruleset
	 */
	public StRuleStats(Instances data, FastVector rules, boolean debug) {
		this();
		//m_Data = data;
		m_Ruleset = rules;
		m_Debug = debug;
...
//	}
	 ...
	 */
	public void countData(Instances data) {
		...
		//Instances data = m_Data;
		((StRip.RipperRule)m_Ruleset.elementAt(0)).clear(data,false);
		for (int i = 0; i < size; i++) {
			double[] stats = new double[6]; // 6 statistics parameters
			double[] classCounts = new double[data.classAttribute().numValues()];
			//Instances[] filtered = 
			((StRip.RipperRule)m_Ruleset.elementAt(i)).apply(m_Ruleset, data, i);
	 ...
	 */
	public void countData(int index, Instances data, double[][] prevRuleStats) {
		if(m_Ruleset==null)
			return;
		...
		for (int j = index; j < size; j++) {
			double[] stats = new double[6]; // 6 statistics parameters
			((StRip.RipperRule)m_Ruleset.elementAt(j)).apply(m_Ruleset,data,j);
	 ...
	 */
	private int computeSimpleStats(int index, Instances insts, double[] stats,
			double[] dist) {
		Rule rule = (Rule) m_Ruleset.elementAt(index);
		Attribute predClass = insts.attribute("predicted_class");
		Attribute Covered = insts.attribute("covered");
		...
		StRip.RipperRule ripperRule = (StRip.RipperRule) m_Ruleset.elementAt(index);
		//ripperRule.clear(insts, false);
		ripperRule.apply(insts, true);
		StRip.transitiveClosure(insts, predClass);
	 ...
	 *          rule to be added
	 */
	public void addAndUpdate(Rule lastRule, Instances data) {
		if (m_Ruleset == null)
			m_Ruleset = new FastVector();
		m_Ruleset.addElement(lastRule);

		//Instances data = m_Data;// (m_Filtered == null) ? m_Data : ((Instances[])
														// m_Filtered.lastElement())[1];
		double[] stats = new double[6];
		double[] classCounts = new double[data.classAttribute().numValues()];
		// Instances[] filtered =
		data = ((StRip.RipperRule)lastRule).apply(m_Ruleset,data,m_Ruleset.size()-1);
	...
	public double potential(int index, double expFPOverErr, double[] rulesetStat,
		...
		if (m_Debug) {
			System.err.println("From potential: "
					+ ((StRip.RipperRule) m_Ruleset.elementAt(index)).toString(null));
	 ...
	 *          the minDataDL
	 */
	public double minDataDLIfDeleted(Instances data, int index, double expFPRate, boolean checkErr) {
		...
		//Instances data = (index == 0) ? m_Data : ((Instances[]) m_Filtered
				//.elementAt(index - 1))[1];
		//Instances data = m_Data;
		// System.out.println("!!!without: " + data.sumOfWeights());
		((StRip.RipperRule)m_Ruleset.elementAt(index)).apply(m_Ruleset,data,index);
		for (int j = (index + 1); j < m_Ruleset.size(); j++) {
			...
			((StRip.RipperRule)m_Ruleset.elementAt(index)).applyWhileSkipping(m_Ruleset,data,index,j+1);
		}
	 ...
	 */
	public double relativeDL(Instances data, int index, double expFPRate, boolean checkErr) {
	 ...
	 *          whether check if error rate >= 0.5
	 */
	public void reduceDL(Instances data, double expFPRate, boolean checkErr) {
	...
	public static Instances rmCoveredBySuccessives(Instances data,
		...
		Instances rt = new ModifiedInstances(data, 0);
	...
	public static final Instances stratify(Instances data, int folds, Random rand) {
		...
		Instances result = new ModifiedInstances(data, 0);
		Instances[] bagsByClasses = new ModifiedInstances[data.numClasses()];

		for (int i = 0; i < bagsByClasses.length; i++)
			bagsByClasses[i] = new ModifiedInstances(data, 0);
	 ...
	 * @return the stratified instances
	 */
	public static final Instances randomizeByDoc(Instances data, int folds,
			Random rand) {
		if (!data.classAttribute().isNominal())
			return data;
		...
		Instances result = new ModifiedInstances(data, 0);
		Instances[] bagsByDocument = new ModifiedInstances[folds];

		for (int i = 0; i < folds; i++)
			bagsByDocument[i] = new ModifiedInstances(data, 0);

		int startNextDoc = 0;
		for (int i = 0; i < numDocs; i++) {
			startNextDoc = ((ModifiedInstances) bagsByDocument[index[i]]).addDoc(
					data, startNextDoc);
		}
		...
		// System.err.println("Randomized: " + result.toString(false));
		//System.err.println("Randomized: "+((ModifiedInstances)result).printDocOrder());
		return result;
	}
	
	/**
	 * Randomize the given data into the given number of bags while keeping
	 * instances that belong to the same document togther and partition the 
	 ...
	 * @return the stratified instances
	 */
	public static final Instances[] randomizeAndPartitionByDoc(Instances data, int folds,
			Random rand) {
		...
		result[0] = new ModifiedInstances(data, 0);
		result[1] = new ModifiedInstances(data, 0);
		Instances[] bagsByDocument = new ModifiedInstances[folds];
		...
		int startNextDoc = 0;
		for (int i = 0; i < numDocs; i++) {
			startNextDoc = ((ModifiedInstances) bagsByDocument[index[i]]).addDoc(data, startNextDoc);
		}

		//The first numFolds - 1 folds go in the first part 
		for (int k = 0; k < folds-1; k++) {
			for (int j = 0; j < bagsByDocument[k].numInstances(); j++)
				result[0].add(bagsByDocument[k].instance(j));
		}
		//The last fold goes in the second part
		int k = folds -1;
		for (int j = 0; j < bagsByDocument[k].numInstances(); j++)
			result[1].add(bagsByDocument[k].instance(j));

		// System.err.println("Randomized: " + result.toString(false));
		//System.err.println("Randomized: "+ ((ModifiedInstances)result[0]).printDocOrder());
		return result;
	}
	 ...
	 */
	public double combinedDL(Instances data, double expFPRate, double predicted) {
		double rt = 0;
	...
	public static final Instances[] partition(Instances data, int numFolds) {
		Instances[] rt = new ModifiedInstances[2];
		int splits = data.numInstances() * (numFolds - 1) / numFolds;

		rt[0] = new ModifiedInstances(data, 0, splits);
		rt[1] = new ModifiedInstances(data, splits, data.numInstances() - splits);
	 ...
	 * @return the patitioned instances
	 */
	public static final Instances[] partitionByDocument(Instances data,
			int numFolds) {
		Instances[] rt = new ModifiedInstances[2];
		int numDocuments = data.getNumDocuments();
		int splits = numDocuments * (numFolds - 1) / numFolds;

		rt = ((ModifiedInstances) data).copyDocInstances(splits);
	...
	public String toString() {
		...
		for (int i = 0; i < m_SimpleStats.size(); i++) {
			result += ((StRip.RipperRule) m_Ruleset.elementAt(i)).toString() + "\n";
	...
	public String toRIPPERString() {
		...
		for (i = 0; i < m_Ruleset.size(); i++) {
			StRip.RipperRule rule = ((StRip.RipperRule) m_Ruleset.elementAt(i));
			result += "+";
