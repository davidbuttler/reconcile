...
public class MultiBoostAB extends AdaBoostM1 {
  ...
  protected void setWeights(Instances training, double reweight) 
	...
	training.instance(i).setWeight((float)(- Math.log((m_Random.nextDouble() * 9999) / 10000) ));
      }

      // Renormailise weights
      double sumProbs = training.sumOfWeights();
      for (int i = 0; i < training.numInstances(); i++) {
	training.instance(i).setWeight((float)(training.instance(i).weight() * oldSumOfWeights / sumProbs));
      }
    } else {
