...
import reconcile.weka.classifiers.functions.supportVector.Kernel;
import reconcile.weka.classifiers.functions.supportVector.NormalizedPolyKernel;
import reconcile.weka.classifiers.functions.supportVector.PolyKernel;
import reconcile.weka.classifiers.functions.supportVector.RBFKernel;
import reconcile.weka.classifiers.functions.supportVector.SMOset;
...
public class SMO extends Classifier implements WeightedInstancesHandler {
  ...
  protected class BinarySMO implements Serializable {
	...
	for (int i = 0; i < numFolds; i++) {
	  ...
	  for (int j = 0; j < test.numInstances(); j++) {
	    float[] vals = new float[2];
	    vals[0] = (float)smo.SVMOutput(-1, test.instance(j));
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
	...
	newInst[0] = (float)m_classifiers[0][1].SVMOutput(-1, inst);
	...
	for (int j = i + 1; j < inst.numClasses(); j++) {
	  if ((m_classifiers[i][j].m_alpha != null) || 
	      (m_classifiers[i][j].m_sparseWeights != null)) {
	    float[] newInst = new float[2];
	    newInst[0] = (float)m_classifiers[i][j].SVMOutput(-1, inst);
	    newInst[1] = Instance.missingValue();
