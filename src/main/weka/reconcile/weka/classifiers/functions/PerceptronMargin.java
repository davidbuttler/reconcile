...
public class PerceptronMargin extends Classifier implements OptionHandler{
	/** The similarity threshold */
	private static double THRESHOLD = 0;
	/** base directory for saving the coref chains to file;
	 *  this is for testing and verification.
	 *  - dah 10/16/07
	 */
	private static String m_corefchain_dir = "";

	private static boolean HYSOM = false;
	...
	/** Ratio of pos/neg examples */
	private static double m_NegWeight = 1;
	...
	private ModifiedInstancesShort[] m_TrueTest;
	...
	private static float margin = (float)1;
	/**
	...
	public double[] evaluateClassifier(ModifiedInstancesShort[] insts, double[] w, LeanDocument[] truth) throws Exception {
		return evaluateClassifier(insts, w, truth, false);
	}

	public double[] evaluateClassifier(ModifiedInstancesShort[] insts, double[] w, LeanDocument[] truth, boolean outputMistakes)
	throws Exception {
		if(insts == null)
			return null;
		int vecLen = 0;
		...
		int totalNumNPsRecall = 0, totalNumNPsPrecision = 0;
		...
		for(int d = 0; d < insts.length; d++){
			...
			UnionFind ptrs = apply(w, doc);
			int[] pred = positiveIdentification(ptrs, doc, outputMistakes);
			LeanDocument cDoc = makeDocument(doc, ptrs);
//			if(doc.instance(0).value(doc.m_DOCNO)==127){
...
//			}
		...
		//System.out.println("Bcubbed comp "+totalPrecision+"/"+totalNumNPsPrecision+"|"+totalRecall+"/"+totalNumNPsRecall);
	...
	throws Exception {
		if(insts == null)
			return null;
		//System.err.println("Train "+insts[0].instance(0));
		//System.err.println("Test "+testInsts==null?0:testInsts[0].instance(0));
		if(!m_StandardUpdate){
		...
		System.err.println("Delta is " + m_delta + ". Threshold is " + THRESHOLD + ". Pos weight is "
				+ m_PosWeight + ". Neg weight "+m_NegWeight+". Margin is "+margin+". Epsilon "+epsilon);
		int vecLen = 0;
		//get the lenght of the feature vectors
		for(int i = 0; i < insts.length && vecLen == 0; i++){
			ModifiedInstancesShort ins = insts[i];
		...
		//The weight vector, w
		double[] w = new double[vecLen], totalW = new double[vecLen], averageW=new double[vecLen];
		int totalUpdates = 0, totalPositive = 0, totalNegative=0;
		if(m_Weights == null){
		...
		for(int iter = 0; iter < numIterations; iter++){
			...
			for(int d = 0; d < insts.length; d++){
				...
				//  System.out.print(d+"...");
				ModifiedInstancesShort doc = insts[d];
				//LeanDocument cDoc = apply(w,doc);
				UnionFind clusterPtrs = apply(w, doc);
				for(int i = 0; i < doc.numInstances(); i++){
					InstanceShort inst = doc.instance(i);
					boolean changed = false;
					//System.err.println(inst.classValue()+":"+doc.m_Positive);
					boolean positive = inst.classValue() == doc.m_Positive;
					short id1 = inst.value(doc.m_ID1);
					short id2 = inst.value(doc.m_ID2);
					if(m_StandardUpdate){
						//Just the traditional perceptron update
						float exWeight = inst.weight();
						if(!positive)
							exWeight = -exWeight;
						double product = innerProduct(w, inst);
						boolean posPrediction =  product + exWeight > margin;
						boolean negPrediction = -product - exWeight > margin;
						if(positive){
							if(!posPrediction){
								changed = true;
								/*
								 * for(int g = 0; g < m_PosWeight; g++){
								 * //System.err.println("Adding "+d+" "+i); if(innerProduct(w,
								 * inst) <= THRESHOLD){ addWeight(w, inst, delta);
								 * positiveUpdates++; } totalUpdates++; addToArray(totalW, w); } }
								 * else{
								 */
								addWeight(w, inst, m_PosWeight * delta);
								positiveUpdates++;
								...
								if(product<=THRESHOLD)
									excluded++;//System.err.println(id1+","+id2+":"+product+":"+exWeight);
						} else{
							if(!negPrediction){
								changed = true;
								subtractWeight(w, inst, delta);
								negativeUpdates++;
								totalUpdates++;
								addToArray(totalW, w);
						...
						if(positive){
							...
							if(clusterPtrs.find(id1) != clusterPtrs.find(id2)){
								//Update the weight vector to correct the mistake
								changed = true;
								/*for(int g = 0; g < m_PosWeight; g++){
                  //System.err.println("Adding "+d+" "+i);
                  if(find(id1, clusterPtrs) != find(id2, clusterPtrs)){
                    addWeight(w, inst, delta);
                    clusterPtrs = apply(divideArray(w, totalUpdates), doc);
                    positiveUpdates++;
                  }
                  totalUpdates++;
                  addToArray(totalW, w);
							...
							if(clusterPtrs.find(id1) == clusterPtrs.find(id2)){
								if(inst.value(doc.m_Prediction) == doc.m_Positive){
									...
									if(m_StandardWeight){
										...
										int numItems = getChainSize(id1, clusterPtrs);
										if(m_NegUpdate1){
											int updated = 0;
											do{
												subtractWeight(w, inst, delta);
												...
												changed = true;
												updated++;
											} while(updated < numItems && inst.value(doc.m_Prediction) == doc.m_Positive);
										} else{
											subtractWeight(w, inst, numItems * delta / m_NegWeight);
											clusterPtrs = apply(divideArray(w, totalUpdates), doc);
											negativeUpdates++;
											totalUpdates++;
											addToArray(totalW, w);
			...
			averageW = divideArray(totalW, totalUpdates);
			if(m_FullOutput)
			...
			if(m_FullOutput)
				...
				System.out.print(positiveUpdates + "," + negativeUpdates + "("+excluded+")=" + updts + ";");
			...
			if(!m_FullOutput)
				System.out.println();
			totalPositive+=positiveUpdates;
			totalNegative+=negativeUpdates;
		}
		...
		if(m_FullOutput){
			...
			System.out.println("The final w is " + printWeightVector(averageW));
			System.out.print("Final on test data: ");


			evaluateClassifier(insts, averageW, truth);
		}
		double[] finalW = divideArray(w, totalPositive+totalNegative);

		double tn = twoNorm(averageW);
		System.out.println("The final w is " + printWeightVector(averageW,insts[0])+"; |w|="+tn);
		System.out.println("Total updates (pos/neg): "+totalPositive+"-"+totalNegative);
		//System.out.println("Maximum feature weights: pos="+totalPositive/(double)totalUpdates+" neg="+totalNegative/(double)totalUpdates);

//		System.out.print("Final on test data: ");
//		tn = tn/4;
//		double increment = tn/20.0;
//		for(double th=0; th<=tn; th+=increment){
//		THRESHOLD=-th;
//		System.out.print("Threshold="+THRESHOLD+":");
//		evaluateClassifier(insts, averageW, truth, false);
//		evaluateClassifier(testInsts, averageW, null, false);

//		if(m_TrueTest!=null){
//		System.out.print("\nTrue test:");
//		VectorComparator.evaluateClassifier(testInsts, m_TrueTest, false, true);
...
//		}
		...
		if(testInsts!=null)
			for(int i=0; i<testInsts.length; i++)
				setPredClass(w, testInsts[i]);
		averageW = unnormalizeWeightVector(averageW,insts[0]);
		averageW = normalizeVector(averageW);

		return averageW;
	}

	public static double twoNorm(double[] w){
		double result = 0;
	...
	public double[] divideArray(double[] w, double divider) {
		double[] result = new double[w.length];
		for(int i = 0; i < w.length; i++)
			result[i] = w[i] / divider;
		return result;
	...
	public double[] unnormalizeWeightVector(double[] w, InstancesShort data) {
		double[] result = new double[w.length];
		for (int i = 0; i < w.length; i++) {
			if(data.attribute(i).isNumeric()){
				double normalizer = (double)data.attribute(i).m_NormalizingConstant;
				result[i] = normalizer==0?w[i]:w[i]/normalizer;
			}else
				result[i] = w[i];
	...
	public double[] normalizeVector(double[] w) {
		double norm = twoNorm(w);
		double[] result = divideArray(w, norm);
		return result;
	}	
	...
	public String printWeightVector(double[] w, InstancesShort data) {
		String result = "";
		NumberFormat nf = NumberFormat.getNumberInstance();
		nf = NumberFormat.getNumberInstance();
		nf.setMinimumFractionDigits(2);
		...
		for (int i = 0; i < w.length; i++) {
			...
			AttributeShort at = data.attribute(i);

			result += at.name() + ": " + nf.format(w[i]);
			if(at.isNominal()){
				result+="(0="+at.value(0)+"|1="+at.value(1)+")";
				/*result+=" (";
				Enumeration enu = at.enumerateValues();
				while (enu.hasMoreElements()) {
					String el = (String)enu.nextElement();
					result += el +"="+at.indexOfValue(el);
	...
	public static int[] positiveIdentification(UnionFind pred, ModifiedInstancesShort gold, boolean outputMistakes) {
		...
		for(int i = 0; i < gold.numInstances(); i++){
			...
			if(positive)
				accurate[2]++;
			if(pred.find(id1) == pred.find(id2)){
				accurate[1]++;
				if(positive)
					accurate[0]++;
				if(outputMistakes)
				...
				System.out.println(id1+"-"+id2+"Predicted -; true +");
		}
	...
	public static double[] main(String[] args,Reader trainUnbufReader,Reader testUnbufReader) {
		try{
			double[] w = null;
			String[] options = args;
			PerceptronMargin classifier = new PerceptronMargin();
			...
			String trainFileName, testFileName, trueTestFileName, sourceClass, classIndexString, seedString, foldsString, objectInputFileName, objectOutputFileName, attributeRangeString;
			...
			StringBuffer text = new StringBuffer();
			BufferedReader trainReader = null, testReader = null, trueTestReader = null;
			...
			trueTestFileName = Utils.getOption('Q', options);
			...
			try{
				if(trainFileName.length() != 0){
					trainReader = trainUnbufReader==null?new BufferedReader(new FileReader(trainFileName)):new BufferedReader(trainUnbufReader);
				}
				if(testFileName.length() != 0){
					testReader = testUnbufReader==null?new BufferedReader(new FileReader(testFileName)):new BufferedReader(testUnbufReader);
				}
				if(trueTestFileName.length() != 0){
					trueTestReader = new BufferedReader(new FileReader(trueTestFileName));
				}
			} catch(Exception e){
			...
			trainAll.setClass(trainAll.attribute("class"));
			trainAll.setPositiveClass(trainAll.attribute("class").indexOfValue("+"));
			//System.err.println(trainAll);
			...
			if(testReader!=null){
				...
				test.cleanUpValuesAndSetWeight(0);
				test.insertAttributeAt(predClass, test.numAttributes());
				//System.err.println(trainAll);
				int num = test.getNumDocuments();
				test = test.binarizeValues();
				//System.out.println("Test "  + test.getNumDocuments() + " docs.");
				test.normalizeAttrValues(data[0]);
				test.setClass(trainAll.attribute("class"));
				test.setPositiveClass(trainAll.attribute("class").indexOfValue("+"));
				test.setNumDocuments(num);
				testData = ModifiedInstancesShort.splitDocs(test);
				trainData = data;
				System.out.println("Train " + trainAll.numInstances() + " insts.");
				System.out.println("Test "  + test.numInstances() + " insts.");

			} else {
				testData = null;
				trainData = data;
				System.out.println("Train " + trainAll.numInstances() + " insts.");
				System.out.println("No test during training.");
...
//				+ testInst + " insts.");
			...
			if(m_EvaluateOnly){
				...
				for(int i = 0; i < truth.length; i++){
					truth[i] = classifier.makeDocument(testData[i]);
					//System.err.println(truth[i]);
				}
				classifier.evaluateClassifier(testData, m_Weights, truth);

			} else{
				if(trueTestReader!=null){
					ModifiedInstancesShort trueTest = new ModifiedInstancesShort(trueTestReader);
					System.err.println("\nRead trueTest");
					trueTest.setClass(trueTest.attribute("class"));
					trueTest.cleanUpValuesAndSetWeight(0);
					int num = trueTest.getNumDocuments();
					System.err.println("Cleaned up");
					trueTest.setClass(trueTest.attribute("class"));
					trueTest.setPositiveClass(trueTest.attribute("class").indexOfValue("+"));
					trueTest.setNumDocuments(num);
					ModifiedInstancesShort[] trueTestData = ModifiedInstancesShort.splitDocs(trueTest);
					classifier.m_TrueTest = trueTestData;
					System.out.println("trueTest "  + trueTestData.length + " docs. "+trueTest.classAttribute());

				}
				w = classifier.buildClassifier(trainData, testData);
				if(m_SaveFilename.length() != 0){
					try{
						PrintStream file = new PrintStream(new FileOutputStream(m_SaveFilename));
						for(int y = 0; y < w.length; y++)
							if(!trainData[0].attribute(y).equals(predClass))
								file.println(w[y]);
					} catch(IOException ioe){
						throw new RuntimeException(ioe);
					}
				...
				if(testData!=null)
					classifier.evaluateClassifier(testData, w, null);
				//System.out.println("Weights: ");
				System.out.println("Two norm="+twoNorm(w));
				classifier.printWeightVector(w, data[0]);
			}
			System.out.println();
			return(w);
		} catch(Exception e){
	...
	public UnionFind apply(double[] w, ModifiedInstancesShort data) {
		...
		int len = (id1>id2?id1:id2) + 1;
		ptrs = new UnionFind(len);
	...
	public static void setPredClass(double[] w, ModifiedInstancesShort data) {
		...
		for(int i = 0; i < data.numInstances(); i++){
			...
			if(innerProduct(w, current) > THRESHOLD){
				current.setValue(data.attribute("predicted_class"), data.m_Positive);
			} else{
				current.setValue(data.attribute("predicted_class"), data.m_Negative);
			}
		}
	}

	private static LeanDocument makeDocument(ModifiedInstancesShort data, UnionFind ptrs) {
		LeanDocument result = new LeanDocument();
		AttributeShort ID1 = data.m_ID1;
		AttributeShort ID2 = data.m_ID2;
		...
		for(int k = 0; k < data.numInstances(); k++){
			...
			if(!result.contains(new Integer(curId1))){
				...
				result.add(new Integer(curId1), new Integer(ptrs.find(curId1)));
			}
			if(!result.contains(new Integer(curId2)))
				result.add(new Integer(curId2), new Integer(ptrs.find(curId2)));
		}
		return result;
	}
	...
	private class UnionFind{
		...
		public int find(int i) {
			...
			if(i>=ptrs.length){
				int[] newPtrs = new int[ptrs.length*2];
				for(int j=0;j<ptrs.length; j++)
					newPtrs[j]=ptrs[j];
				for(int j=ptrs.length;j<newPtrs.length; j++)
					newPtrs[j]=j;
				ptrs = newPtrs;
			}
	...
	private int getChainSize(int id1, UnionFind clusterPtrs) {
		int cl = clusterPtrs.find(id1);
		int num = 0;
		for(int i = 0; i < clusterPtrs.size(); i++)
			if(clusterPtrs.find(i) == cl)
				num++;
		return num;
	}

	public LeanDocument makeDocument(ModifiedInstancesShort data) {
		if(data == null || data.numInstances() == 0)
			return null;
		LeanDocument result = new LeanDocument(data.instance(0).value(data.m_DOCNO));
		AttributeShort DOCID = data.m_DOCNO;
		//System.err.println("Working on document "+DOCID);
		AttributeShort ID1 = data.m_ID1;
		AttributeShort ID2 = data.m_ID2;
