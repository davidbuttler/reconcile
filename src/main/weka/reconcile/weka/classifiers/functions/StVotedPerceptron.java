...
public class StVotedPerceptron extends Classifier implements OptionHandler{
  ...
  private double[][] m_Additions = null;
  ...
  public double[] evaluateClassifier(ModifiedInstancesShort[] insts, LeanDocument[] truth)
      throws Exception {
    if(insts == null)
      return null;
    int vecLen = 0;
   ...
   */
  public void buildClassifier(ModifiedInstancesShort[] insts, ModifiedInstancesShort[] testInsts)
      throws Exception {
    if(insts == null)
      return;
    if(!m_StandardUpdate){
    ...
    int vecLen = 0;
    m_Additions = new double[m_MaxK][];
    m_Weights = new double[m_MaxK];
    //get the lenght of the feature vectors
    for(int i = 0; i < insts.length && vecLen == 0; i++){
      ModifiedInstancesShort ins = insts[i];
      if(ins != null){
    ...
    out:
    for(int iter = 0; iter < numIterations && m_K<m_MaxK; iter++){
      ...
      for(int d = 0; d < insts.length; d++){
        ...
        int[] clusterPtrs = apply(divideArray(w, totalUpdates), doc);
      ...
      StPerceptron.evaluateClassifier(insts,w, truth);

      //System.out.println("After iter "+iter+" w is "+printWeightVector(w));
      if(testInsts != null){
        double metrics[] = StPerceptron.evaluateClassifier(testInsts, w, testTruth);
        if(maxAcc < metrics[0]){
          maxAcc = metrics[0];
          maxAccIter = iter;
        }
      ...
      System.out.println("Final on test data: " + m_K + " perceptrons");

      evaluateClassifier(insts, truth);
  }
  ...
  private double prediction(InstanceShort i) {
    double result = 0;
    for(int j = 0; j < m_K; j++){
      if(innerProduct(m_Additions[j],i)>THRESHOLD)
        result+=m_Weights[j];
      else
        result-=m_Weights[j];
    }
  ...
  public static void main(String[] args) {
    try{
      String[] options = args;
      StVotedPerceptron classifier = new StVotedPerceptron();
      ...
      System.out.println("Split at document " + mid + ". Train " + trainInst + " insts. Test "
          + testInst + " insts.");
      classifier.buildClassifier(trainData, testData);
      //classifier.evaluateClassifier(trainData,w);
      classifier.evaluateClassifier(testData, null);
    } catch(Exception e){
      e.printStackTrace();
      System.err.println(e.getMessage());
    }
  ...
  public int[] apply(ModifiedInstancesShort data) {
