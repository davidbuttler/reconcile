...
public class MultilayerPerceptron extends Classifier 
	...
	if (noa != inst.classIndex() && m_normalizeAttributes) {
	  for (int i = 0; i < inst.numInstances(); i++) {
	    if (m_attributeRanges[noa] != 0) {
	      inst.instance(i).setValue(noa, (float)((inst.instance(i).value(noa)  
					      - m_attributeBases[noa]) /m_attributeRanges[noa]));
	    }
	    else {
	      inst.instance(i).setValue(noa, (float)(inst.instance(i).value(noa) - 
  ...
  public double[] distributionForInstance(Instance i) throws Exception {
	...
	if (noa != m_instances.classIndex()) {
	  if (m_attributeRanges[noa] != 0) {
	    m_currentInstance.setValue(noa, (float)((m_currentInstance.value(noa) - 
					     m_attributeBases[noa]) / m_attributeRanges[noa]));
	  }
	  else {
	    m_currentInstance.setValue(noa, (float)(m_currentInstance.value(noa) -
