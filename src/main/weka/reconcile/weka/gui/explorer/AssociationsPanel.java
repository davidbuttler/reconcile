...
import reconcile.weka.associations.Associator;
...
public class AssociationsPanel extends JPanel {
  ...
  static {
    ...
    java.beans.PropertyEditorManager
      .registerEditor(reconcile.weka.associations.Associator.class,
		      reconcile.weka.gui.GenericObjectEditor.class);
  }
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    m_AssociatorEditor.setValue(new reconcile.weka.associations.Apriori());
  ...
  public void run() {
	  ...
	  String cname = associator.getClass().getName();
        if (cname.startsWith("reconcile.weka.associations.")) {
          name += cname.substring("reconcile.weka.associations.".length());
	  } else {
	    name += cname;
	  }
	  try {
