...
import reconcile.weka.clusterers.EM;
...
public class Clusterer extends JPanel implements BeanCommon, Visible, WekaWrapper, EventConstraints, Serializable, UserRequestAcceptor, TrainingSetListener, TestSetListener{
  ...
  private reconcile.weka.clusterers.Clusterer m_Clusterer = new EM();
   ...
   */
  public void setClusterer(reconcile.weka.clusterers.Clusterer c) {
   ...
   */
  public reconcile.weka.clusterers.Clusterer getClusterer() {
    return m_Clusterer;
  }
  ...
  public void setWrappedAlgorithm(Object algorithm) 
    ...
    if (!(algorithm instanceof reconcile.weka.clusterers.Clusterer)) { 
      throw new IllegalArgumentException(algorithm.getClass()+" : incorrect "
					 +"type of algorithm (Clusterer)");
    }
    setClusterer((reconcile.weka.clusterers.Clusterer)algorithm);
  }
	  ...
	  m_buildThread = new Thread() {
		...
		try {
		  if (m_trainingSet != null) {  
		    ...
		    if (m_Clusterer instanceof reconcile.weka.core.Drawable && 
			m_graphListeners.size() > 0) {
		      String grphString = 
			((reconcile.weka.core.Drawable)m_Clusterer).graph();
                      int grphType = ((reconcile.weka.core.Drawable)m_Clusterer).graphType();
