...
public class PredictionAppender extends JPanel
  ...
  private Instances 
    makeDataSetProbabilities(Instances format,
			     reconcile.weka.classifiers.Classifier classifier,
  ...
  throws Exception {
    ...
    for (int i = 0; i < format.classAttribute().numValues(); i++) {
      reconcile.weka.filters.unsupervised.attribute.Add addF = new
	reconcile.weka.filters.unsupervised.attribute.Add();
  ...
  throws Exception {
    ...
    Instances newInstances = 
      reconcile.weka.filters.Filter.useFilter(format, addF);
    newInstances.setRelationName(format.relationName()+relationNameModifier);
    return newInstances;
  }
  ...
  private Instances 
    makeClusterDataSetProbabilities(Instances format,
			     reconcile.weka.clusterers.Clusterer clusterer,
			     String relationNameModifier) 
  throws Exception {
    int numOrigAtts = format.numAttributes();
    Instances newInstances = new Instances(format);
