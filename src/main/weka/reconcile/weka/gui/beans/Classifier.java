...
public class Classifier extends JPanel
  ...
  private reconcile.weka.classifiers.Classifier m_Classifier = new ZeroR();
   ...
   */
  public void setClassifier(reconcile.weka.classifiers.Classifier c) {
    ...
    if (!(m_Classifier instanceof reconcile.weka.classifiers.UpdateableClassifier) &&
   ...
   */
  public reconcile.weka.classifiers.Classifier getClassifier() {
    return m_Classifier;
  }
  ...
  public void setWrappedAlgorithm(Object algorithm) 
    ...
    if (!(algorithm instanceof reconcile.weka.classifiers.Classifier)) { 
      throw new IllegalArgumentException(algorithm.getClass()+" : incorrect "
					 +"type of algorithm (Classifier)");
    }
    setClassifier((reconcile.weka.classifiers.Classifier)algorithm);
  }
  ...
  private void handleIncrementalEvent() {
	...
	if (m_trainingSet == null || (!dataset.equalHeaders(m_trainingSet))) {
	  if (!(m_Classifier instanceof 
		reconcile.weka.classifiers.UpdateableClassifier)) {
	    if (m_log != null) {
	...
	status = IncrementalClassifierEvent.BATCH_FINISHED;
	  ...
	  !(m_incrementalEvent.getInstance().
	    isMissing(m_incrementalEvent.getInstance().
		      dataset().classIndex()))) {
	((reconcile.weka.classifiers.UpdateableClassifier)m_Classifier).
	  ...
	  m_buildThread = new Thread() {
		...
		try {
		  if (m_trainingSet != null) {
		    ...
		    if (m_Classifier instanceof reconcile.weka.core.Drawable && 
			m_graphListeners.size() > 0) {
		      String grphString = 
			((reconcile.weka.core.Drawable)m_Classifier).graph();
                      int grphType = ((reconcile.weka.core.Drawable)m_Classifier).graphType();
