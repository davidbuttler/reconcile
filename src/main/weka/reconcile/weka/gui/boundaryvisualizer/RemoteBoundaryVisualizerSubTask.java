...
import reconcile.weka.experiment.Task;
...
public class RemoteBoundaryVisualizerSubTask implements Task {
  ...
  public void execute() {
	...
	throw new Exception("Visualization dimensions must be numeric "
      ...
      m_predInst = new Instance(1, m_vals);
  ...
  private double [] calculateRegionProbs(int j, int i) throws Exception {
	...
	for (int q = 0; q < values.length; q++) {
	  if (values[q] != null) {
	    ...
	    m_vals[m_xAttribute] = (float)m_weightingAttsValues[m_xAttribute];
	    m_vals[m_yAttribute] = (float)m_weightingAttsValues[m_yAttribute];
