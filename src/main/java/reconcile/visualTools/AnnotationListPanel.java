...
public AnnotationListPanel(MainWindow mainw) {
  ...
  for (AnnotationSet as : anSets) {
    model.addElement("================ " + as.getName() + " ===============");
    for (Annotation a : as) {
      model.addElement(a);
    }
  }
  anList.setCellRenderer(new AnnotationCellRenderer());
...
public void redraw()
{
  ...
  model = new DefaultListModel();
  for (AnnotationSet as : anSets) {
    model.addElement("================ " + as.getName() + " (" + as.size() + ") " + " ===============");
    for (Annotation a : as) {
      model.addElement(a);
    }
  }
...
public void valueChanged(ListSelectionEvent e)
{
  ...
  if (lsm.isSelectionEmpty()) {
    ...
    if (maxIndex - minIndex == 0) {
      ...
      Object el = anList.getModel().getElementAt(maxIndex);
      if (el instanceof Annotation) {
        Annotation a = (Annotation) el;
        if (a.getType().equalsIgnoreCase("NP") && a.getAttribute("CorefID") != null) {
          ...
          if (corefID >= 0) {
            ...
            for (int i = 0; i < lm.getSize(); i++) {
              Object curEl = lm.getElementAt(i);
              if (curEl instanceof Annotation) {
                Annotation cur = (Annotation) curEl;
    ...
    for (int i = minIndex; i <= maxIndex; i++) {
      if (lsm.isSelectedIndex(i)) {
        Object el = anList.getModel().getElementAt(i);
        if (el instanceof Annotation) {
          Annotation a = (Annotation) el;
